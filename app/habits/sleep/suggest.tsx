...
export default function SleepSuggestScreen() {
  const router = useRouter();
  const [status, setStatus] = useState<'checking' | 'show'>('checking');

  useEffect(() => {
    AsyncStorage.getItem('habit_enabled_sleep').then((value) => {
      if (value === 'true') {
        router.replace('/habits/sleep');
      } else {
        setStatus('show');
      }
    });
  ...
  const handleEnable = async () => {
    await AsyncStorage.setItem('habit_enabled_sleep', 'true');
    router.replace('/habits/sleep');
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Enable sleep Tracker?</Text>
