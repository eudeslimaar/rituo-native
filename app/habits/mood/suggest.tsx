...
export default function MoodSuggestScreen() {
  const router = useRouter();
  const [status, setStatus] = useState<'checking' | 'show'>('checking');

  useEffect(() => {
    AsyncStorage.getItem('habit_enabled_mood').then((value) => {
      if (value === 'true') {
        router.replace('/habits/mood');
      } else {
        setStatus('show');
      }
    });
  ...
  const handleEnable = async () => {
    await AsyncStorage.setItem('habit_enabled_mood', 'true');
    router.replace('/habits/mood');
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Enable Mood Tracker?</Text>
